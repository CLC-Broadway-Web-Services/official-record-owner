...
if (isset($_REQUEST['delete']) && $_REQUEST['delete'] == 'y') {
  ...
  $sqlDelete = $db->execute("DELETE FROM `project` WHERE `id` = '$id'");
  if ($sqlDelete == true) :
...
if (isset($_REQUEST['status'])) {
  ...
  $sqlStatus = $db->execute("UPDATE `project` SET `status` = '$status' WHERE `id` = '$id'");
}

$cateogryQuery = $function->getAllItems("project");
                ...
                <tbody>
                  ...
                  ?>
                    <tr>
                      ...
                      <td>
                        <a href="add-project.php?id=<?php echo base64_encode($row['id']); ?>"><i class="fa fa-edit"></i></a>
                        ||
