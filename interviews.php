...
// $AllinterviewData = data("interview", $idStatus);
$sql = "SELECT *,interview.id mainId FROM interview  JOIN interview_category ON   interview.category = interview_category.id  where interview.status = 1 ORDER BY interview.id DESC";
$AllinterviewData = $db->getAllData($sql);
$allinterviewCatData = data("interview_category");
				...
				<div class="col-sm-12">
					<div class="gallery-filter">
						<ul class="nav nav-pills masonry-filter">
							...
							<?php foreach ($allinterviewCatData as $key => $item) : ?>
		...
		<div class="container">
			<div class="tab-content">
				<div id="home" class="tab-pane fade in active">
					<div class="row">
						<?php foreach ($AllinterviewData as $key => $item) : ?>
				...
				<?php foreach ($allinterviewCatData as $key => $item) :
					$interviewData = $db->getAllData("SELECT *,interview.id mainId FROM interview  JOIN interview_category ON   interview.category = interview_category.id WHERE interview.category = " . $item['id'] ." AND interview.status = 1")
				?>
					<div id="cat_<?= $item['id'] ?>" class="tab-pane fade">
						<div class="row">
							<?php foreach ($interviewData as $key => $row) : ?>
								<div class="col-md-4 col-sm-6">
									<div class="film-masonry-item-inner">
										<div class="thumb">
