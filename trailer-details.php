...
$trailerData = data("trailer", $idStatus, 2, $id);
$recenttrailer = data("trailer", $idStatus, 3, $id, 10);
$bannerData = data("pages_banner", 1, 2, 3);
$is_video = (!empty($trailerData['youtube'])) ? 1 : 0;
if ($is_video) {
	$youtubeLink = $trailerData['youtube'];
...
<div id="main">
			...
			<div class="row">
				<div class="col-sm-12">
					<div class="film-detail-video">
						...
						<?php } else { ?>
							<img src="record@1357admin/dist/img/<?= ($trailerData['image_detail']) ?>" height="500" width="1170" />
						<?php } ?>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-9">
					<div class="film-detail-title">
						<h3 class="section-title"> <?= $trailerData['heading']; ?> </h3>
					</div>

					<div class="film-detail-content">
						<?= $trailerData['description']; ?>
						<div class="mb-4"></div>
						<img src="record@1357admin/dist/img/<?= $trailerData['image1']; ?>" alt="img" />
						<div class="mb-4"></div>
						<?= $trailerData['description2']; ?>
					</div>
				</div>
				<div class="col-sm-3">
	...
	<div class="section bg-light pt-7 pb-4">
		<div class="container">
			<div class="row">
				<div class="col-sm-12">
					<h3 class="section-title fz-24 fw-bolder bottom-line">Recent Trailers</h3>
					<div class="relate-film-carousel row mt-5" data-auto-play="true" data-desktop="4" data-laptop="3" data-tablet="3" data-mobile="2">
						<?php foreach ($recenttrailer as $key => $item) : ?>
							<div class="item">
								<div class="item-inner">
									<div class="thumb">
										<a href="ftrailer-details.php?id=<?= base64_encode($item['id']) ?>">
											<img src="record@1357admin/dist/img/<?= ($item['image']) ?>" alt="" />
										</a>
									</div>
									<div class="info">
										<div class="title">
											<a href="ftrailer-details.php?id=<?= base64_encode($item['id']) ?>"><?= substr($item['heading'], 0, 20) ?></a>
										</div>
									</div>
								</div>
							</div>
