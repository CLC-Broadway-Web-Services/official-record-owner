...
$allScheduleData = data("schedule", $idStatus);
// print_r($allScheduleData);
// die("check");
...
$pagination->records(count($allScheduleData));

// records per page
$pagination->records_per_page($records_per_page);

// here's the magick: we need to display only the records for the current page
$allScheduleData = array_slice(
	$allScheduleData,                                             //  from the original array we extract
	(($pagination->get_page() - 1) * $records_per_page),    //  starting with these records
	$records_per_page                                       //  this many records
);

$bannerData = data("pages_banner", 1, 2, 16);
?>
<style>
	.page-title {
		...
		<div class="container">
			<div class="row">
				<div class="film-masonry masonry-grid-post" style="position: relative; height: 813.688px;">
					<?php foreach ($allScheduleData as $key => $item) : ?>
						<div class="col-md-4 col-sm-6 film-masonry-item masonry-item anthology-film short-film" style="position: absolute; left: 0px; top: 0px;">
							<div class="film-masonry-item-inner">
								<div class="thumb">
									<a href="record@1357admin/dist/img/<?= $item['pdf']?>">
										<img src="record@1357admin/dist/img/<?= $item['image'] ?>" alt="">
									</a>
								</div>
								<div class="info">
									<div class="title">
										<a href="record@1357admin/dist/img/<?= $item['pdf']?>"><?= $item['heading'] ?></a>
									</div>
								</div>
							</div>
						</div>
